...
| Webpack Internal Chunk Name Disclosed | `SCAN_CHECK_PASSIVE_PER_REQUEST` | Detects webpack bundle responses where the webpackChunk<appname> global leaks an internal application or team identifier; fires at INFORMATION/FIRM on any JavaScript response containing a non-generic chunk name. |
| Webpack Module Federation Share-Scope Manifest Exposed | `SCAN_CHECK_PASSIVE_PER_REQUEST` | Detects webpack 5 Module Federation share-scope manifests and reports exposed package names and versions; elevates to MEDIUM if any @scope is absent from the known-public npm scope allowlist, flagging it as a dependency confusion candidate. |
| Webpack Bundle Likely-Internal Package Names | `SCAN_CHECK_PASSIVE_PER_REQUEST` | Applies naming heuristics to unscoped packages found in webpack Module Federation share-scope manifests to surface likely-internal or proprietary package names; always reports at MEDIUM/TENTATIVE due to elevated false-positive rate. |
